...
FLAGS = flags.FLAGS
flags.DEFINE_string('im_type', 'depth', '')
...
# which split to use (in /nfs)
flags.DEFINE_integer("fold", "1", "")

# CUDA_VISIBLE_DEVICES='2' PYTHONPATH='.:maskrcnn/' python clutter/pipeline.py \
#   --logdir outputs/v3_512_40_flip_depth --im_type depth
...
def tune_params(params, num_epochs=30):
  ...
  for i, params in enumerate(config_list):
    ...
    dataset_train.load('train', FLAGS.im_type, FLAGS.fold)
    dataset_train.prepare()

    # Validation dataset
    dataset_val = ClutterDataset()
    dataset_val.load('test', FLAGS.im_type, FLAGS.fold)
    dataset_val.prepare()
...
def train():
  ...
  # Validation dataset
  dataset_val = ClutterDataset()
  dataset_val.load('val', FLAGS.im_type, FLAGS.fold)
  dataset_val.prepare()
...
def test():
  inference_config, model, dataset_sim_test = prepare_for_test()
  benchmark(inference_config, model, dataset_sim_test)
  inference_config, model, dataset_real_test = prepare_for_test(typ="real")
  benchmark(inference_config, model, dataset_real_test)
