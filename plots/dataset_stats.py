...
parser.add_argument('dataset_dir', help='path to dataset to be analyzed')
parser.add_argument('--indices', default='test', help='indices of dataset to be analyzed')
args = parser.parse_args()

mask_dir = os.path.join(args.dataset_dir, 'modal_segmasks')
if args.indices == 'all':
    N = len([p for p in os.listdir(mask_dir) if p.endswith('.png')])
    indices_arr = np.arange(N)
else:
    indices_arr = np.load(os.path.join(args.dataset_dir, args.indices + '_indices.npy'))
    N = indices_arr.size
...
plt.plot(bins[:-1], np.load('instances_per_im.npy'), 'o-')
ax = plt.gca()
ax.yaxis.set_major_formatter(PercentFormatter(decimals=0))
plt.title('Instances per Image')
plt.xlabel('Number of Instances')
plt.ylabel('Percentage of Images')
plt.legend(['COCO (7.7)', 'WISDOM-Sim ({:.1f})'.format(np.mean(segments)), 'WISDOM-Real (4.9)'])
...
plt.plot(100*bins[1:], np.load('area_per_inst.npy'), 'o-')
ax = plt.gca()
ax.yaxis.set_major_formatter(PercentFormatter(decimals=0))
